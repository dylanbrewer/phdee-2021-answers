# Homework 4 data simulation -- Dylan Brewer
...
outputpath = r'C:\Users\dbrewer30\Dropbox\teaching\Courses\BrewerPhDEnv\Homeworks\phdee-2021-homework\homework4'
...
for i in range(nobs):
    ...
    #for t in range(24,36):
        #shrimp[i,t] = (supshrimpa[i,0] - threatened2[i,0]*treatloss[i,0] + supshrimpb[i,0]*pshrimp[0,t] + shrimptrend[0,t] + shrimpit[i,t])*firmsize[i,0]*0.7
        #salmon[i,t] = (supsala[i,0] + supsalb[i,0]*psalmon[0,t] + saltrend[0,t] + salit[i,t])*firmsize[i,0]*0.3
...
for i in range(nobs):
    ...
    #for t in range(24,36):
        #bycatch[i,t] = (alpha-(d1*threatened2[i,0]))*shrimp[i,t] + (beta-(d2*threatened2[i,0]))*salmon[i,t] - d3*threatened2[i,0] + etai[i,0] + epsilonit[i,t]
...
fishbycatch.to_csv('fishbycatch.csv', index = None, header=True)