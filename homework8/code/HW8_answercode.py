...
outputpath = r'C:\Users\dbrewer30\Dropbox\teaching\Courses\BrewerPhDEnv\Homeworks\phdee-2021-answers\homework8\output'
...
os.chdir(outputpath)
plt.savefig('hw8q2.eps',format='eps')
plt.show()
...
plt.savefig('hw8q3.eps',format='eps')
plt.show()
...
plt.savefig('hw8q4.eps',format='eps')
plt.show()
...
plt.savefig('hw8q5.eps',format='eps')
plt.show()
...
hw8output6 = pd.DataFrame(pd.concat([beta,cis],axis = 1).stack())
hw8output6.columns = ['Question 6']
hw8output6.index = ['Sedan',' ','MPG','','Constant','']
hw8output6 = hw8output6.append(pd.DataFrame([str(ivest.nobs)],index = ['Observations'],columns = ['Question 6']))

hw8output6.to_latex('hw8output6.tex',column_format = 'lc', na_rep = ' ')
